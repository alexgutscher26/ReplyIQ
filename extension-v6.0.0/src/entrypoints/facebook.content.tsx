import '@/entrypoints/facebook-style.content.css'
...
export default defineContentScript({
  ...
  async main(ctx) {
    ...
    const isDarkTheme = document.querySelector('meta[name="color-scheme"]')?.getAttribute('content') === 'dark'
    ...
    const createStatusUi = async () => {
      ...
      activeStatusUi = await createShadowRootUi(ctx, {
        anchor: 'form .x1l90r2v.xyamay9.x1n2onr6',
        append: 'first',
        name: 'replier-status',
        onMount: (container) => {
    ...
    const mountForForms = async () => {
      const forms = document.querySelectorAll('form.x1ed109x.x1n2onr6.xmjcpbm[role="presentation"] > div.xh8yej3:first-of-type')
